...
export default async function handler(
  ...
  try {
    switch (req.method) {
      case "POST":
        if (req.query.register !== undefined) {
          ...
          if (validationError) {
            return res
              .status(400)
              .json({ error: validationError.message || validationError });
          }
      ...
      case "PATCH":
        // Authenticate user
        const authResult2 = await authenticateUser(req, res);
        ...
        if (
          !patchAuthenticatedReq.user?.hasRole ||
          !patchAuthenticatedReq.user.hasRole("admin")
        ) {
        ...
        const updateResult = await UserController.updateUserRoles(
          controllerReq,
          undefined as any
        );
